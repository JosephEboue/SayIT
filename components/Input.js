...
const Input = () => {
  ...
  const addImageToPost = (e) => { // Add images to the image link icon
  ...
  const sendPost = async () => { 
    if (loading) return; // If in loading state, don't return any thing
// Else set loading to true
    setLoading(true);

    const docRef = await addDoc(collection(db, "posts"), { // document reference
      id: session.user.uid,
    ...
    const imageRef = ref(storage, `posts/${docRef.id}/image`); // Image reference
